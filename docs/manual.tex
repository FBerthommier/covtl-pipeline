...
\usepackage[
    ...
    pdfkeywords={articulatory synthesis, coarticulation, polar coordinates, VocalTractLab, COVTL, speech synthesis}
]{hyperref}
...
\date{\small Version 1.0.2 --- September 2026\\[0.2em]
...
\texttt{vocaltractlab-cython} wheel with the JD3 speaker. This manual
...
file formats, and the underlying mechanisms and constants.
\end{abstract}
...
\item \textbf{Synthesizer:} VocalTractLab 2.4 (JD3 speaker) via
...
\item 55 pytest tests: engine non-regression against
      \texttt{regression\_baselines.json} plus CLI/g2p unit tests.
\end{itemize}
...
triangle. /m/ coincides with /b/; only the selector and the velum differ.}
...
\item \textbf{Glides are vowels used as consonants.} \texttt{j} carries
      the coordinates of \texttt{i} (\(0.8, 300^\circ\)),
      \texttt{w} those of \texttt{u} (\(0.6, 60^\circ\)) and
      \texttt{\turnh} those of \texttt{y} (\(0.519, 260.6^\circ\)); they are
...
\texttt{N}    & \((1.20, 60^\circ)\)   & (empty) & derived velar place \\
\texttt{J}    & \((1.20, 300^\circ)\)  & (empty) & derived dorsal place (\texttt{NG}) \\
\texttt{w}    & \((0.60, 60^\circ)\)   & JA, LP, LD, TCX, TCY & glide = \texttt{u} coordinates \\
\texttt{j}    & \((0.80, 300^\circ)\)  & JA, TCX, TCY & glide = \texttt{i} coordinates \\
\texttt{\turnh}    & \((0.519, 260.6^\circ)\) & JA, TCX, TCY & glide = \texttt{y} coordinates \\
...
\item[Cluster rules.] \texttt{adjust\_consonant\_theta} applies the
      ...
      \(3\pi/2 = 270^\circ\) (v1.0.9: the former \(23\pi/16\) override
      left the apical closure open on every speaker, the reference
     jd3 included), and \texttt{g/k} back to
...
\(\mathrm{COEFCEN} = 0.5\) for word-end and syllabic-onset anchors in the
...
\item[\texttt{vtl\_synth.core}] the synthesis engine: notation/parsing,
      ...
      envelope/source modules, and the \texttt{Pipeline} orchestrator
      (26 engine modules).
\item[\texttt{vtl\_synth.cli}] the \texttt{vtl-synth} entry point with
      the subcommands \texttt{run}, \texttt{text-to-wav},
      \texttt{tract-to-mp4}, \texttt{inspect}, \texttt{plot-tract}.
...
\item[\texttt{vtl\_synth.video}] the sagittal renderer
      ...
      (\texttt{encoder.py}: ffmpeg H.264/AAC), and the parameter
      figure tool (\texttt{tract\_figure.py}).
\item[\texttt{vtl\_synth.utils}] the grapheme-to-phoneme module
      (\texttt{g2p.py}, CMUdict).
\end{description}
...
The \texttt{vtl-synth} command exposes five subcommands. The engine
options (COVTL model parameters) are shared by \texttt{run} and
\texttt{text-to-wav}:
...
\texttt{-{}-tcons MS}        & consonant (sub-arc) duration, default \textbf{100\,ms} \\
\texttt{-{}-tvoy MS}         & vowel gesture duration, default \textbf{80\,ms} \\
\texttt{-{}-tpause MS}       & short inter-word pause (space), default \textbf{200\,ms} \\
\texttt{-{}-tpause-long MS}  & long phrase pause (\texttt{|}), default \textbf{320\,ms} \\
\texttt{-f0, -{}-f0 HZ}      & fundamental frequency, default \textbf{102.216\,Hz} (JD3) \\
...
\item \textbf{Consonants}: \texttt{p b t d k g f v s z S Z T D tS dZ m n
      J l L R r j w h \turnh{} C X} (canonical voiced forms of
      Table~\ref{tab:cons} plus their voiceless partners);
...
This section walks through the real v1.0.2 output of the demo phrase
(all numbers measured on the artifacts of a default run).
...
|-- examples/demo.py           minimal usage example
|-- tests/                     55 pytest tests (9 modules)
`-- vtl_synth/
    |-- __init__.py            public API exports (Pipeline, ...)
    |-- core/                  26 engine modules (model, trajectories, source)
    |-- cli/main.py            vtl-synth entry point
    |-- video/                 renderer, encoder, tract_figure
...
\begin{lstlisting}[style=shell]
pip install -e ".[dev]"
pytest -v                      # all 55 tests
pytest tests/test_polar_model.py
pytest -k "syltraj"
\end{lstlisting}

The suite covers the polar model (projections, arcs), the
\texttt{legacy}/\texttt{syl} equivalence and structure tests, glottal
classification, notation/parsing, CLI and g2p, the tract figure, and
end-to-end non-regression against \texttt{regression\_baselines.json}.
