...
Because the per-language vowel targets are calibrated in situ on JD3,
every language-management command carries a guard
(\code{\_ensure\_jd3()}): \code{install}, \code{setlang},
\code{calibrate} and \code{verify} first check the active speaker
and, if it is not jd3, reinstall jd3 (constants + marker + wheel
resource) through \code{install\_speaker.py} before performing the
operation. It is a transport mechanism, not a refusal: synthesis is
never gated, and switching speakers afterwards remains free --- the
speaker swap \emph{carries the active LANG SECTION over} to the new
speaker's constants.

Only \textbf{JD3 follows the pack}. The four other speakers of the
registry (s1, s2, m01, w02) ship and are validated with their
\emph{native} vowel targets; to work in that native regime, keep the
default language (\code{en}) and install the speaker with
\code{install\_speaker.py} (\code{docs/speakers.pdf}). Users who
alternate languages and speakers should install the language first,
then the speaker (workflow A of \code{docs/expressive\_prosody.pdf}).
