# it_lexicon.tsv — provenance et licence

`it_lexicon.tsv` est le lexique de prononciation du g2p italien :
`mot<TAB>prononciation` (IPA Wikipron ou clés moteur), consulté en
priorité absolue par `vtl_synth/utils/g2p_it.py`. Chargeur commun :
`vtl_synth/utils/lexicon_loader.py` (variable d'environnement :
`$COVTL_IT_LEXICON`).

## Source

* Fichier Wikipron : `data/scrape/tsv/ita_latn_broad_filtered.tsv`
* Lien brut :
  <https://raw.githubusercontent.com/CUNY-CL/wikipron/master/data/scrape/tsv/ita_latn_broad_filtered.tsv>
...
* Taille attendue : ~89 000 lignes, ~2,6 Mo.
