# de_lexicon.tsv — provenance et licence

`de_lexicon.tsv` est le lexique de prononciation du g2p allemand :
`mot<TAB>prononciation` (IPA Wikipron ou clés moteur), consulté en
priorité absolue par `vtl_synth/utils/g2p_de.py`. Chargeur commun :
`vtl_synth/utils/lexicon_loader.py` (variable d'environnement :
`$COVTL_DE_LEXICON`).

## Source

* Fichier Wikipron : `data/scrape/tsv/deu_latn_broad_filtered.tsv`
* Lien brut :
  <https://raw.githubusercontent.com/CUNY-CL/wikipron/master/data/scrape/tsv/deu_latn_broad_filtered.tsv>
...
* Taille attendue : ~58 000 lignes, ~2 Mo.
