...
\begin{warning}[v1.0.9 LANG SECTION semantics --- sections of this manual superseded]
Since v1.0.9 (2026-09-23) the installed LANG SECTION is a \emph{marker only} (\code{ACTIVE\_LANG}). The language pack no longer installs per-language vowel targets or effort gains: \code{VOWEL\_TARGETS} and \code{VOWEL\_EFFORT\_GAIN} are the active speaker’s own (defect D24 resolved). The passages of this manual that describe per-language vowel targets, the JD3 guard, and \code{calibrate} describe the pre-1.0.9 behaviour and are superseded; the per-language vowel-triangle figure is historical. See the lang\_pack README and the pipeline CHANGELOG for the current semantics.
\end{warning}
