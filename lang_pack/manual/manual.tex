...
\date{Version 1.0.7 --- September 2026}

\begin{document}

\begin{warning}[v1.2.0 LANG SECTION semantics --- sections of this manual superseded]
Since v1.2.0 (2026-09-23) the installed LANG SECTION is a \emph{marker only} (\code{ACTIVE\_LANG}). The language pack no longer installs per-language vowel targets or effort gains: \code{VOWEL\_TARGETS} and \code{VOWEL\_EFFORT\_GAIN} are the active speaker’s own (defect D24 resolved). The passages of this manual that describe per-language vowel targets, the JD3 guard, and \code{calibrate} describe the pre-v1.2.0 behaviour and are superseded; the per-language vowel-triangle figure is historical. See the lang\_pack README and the pipeline CHANGELOG for the current semantics.
\end{warning}
...
main repository manual (\code{docs/manual.pdf}).
\end{minipage}
\end{center}
...
\begin{verbatim}
lang_pack/
|-- profiles/          g2p gateways + language profiles  -> vtl_synth/utils/
...
\item \code{python setup\_lang.py install --lang <xx>}
      --- copies the eleven profile modules into
...
as published with the v1.0.7 blocks. \emph{fr}: the v1.0.6
``validated'' block measures 4/12 under the current chain (see the
LPC bias below). \emph{en}: covered by the 62 historical regression
baselines rather than by the LPC chain (its targets predate the
verification tool).}
\label{tab:verdicts}
\begin{tabular}{lll}
\toprule
...
fr & 4/12 & documented LPC-measurement gap (v1.0.6 block) \\
...
targets between languages was tried and rejected during the v1.0.6
calibration; the pipeline therefore carries one derivation per
language and refuses to guess.

\subsection{Calibrating a language on the engine}

Two commands derive $\rho$ mechanically (never by hand):
